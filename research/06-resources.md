...
The earlier negative results stand and are not retracted: feedback-based calibration training did repeatedly fail in kindergarten, first grade and 7–8 year olds. They simply sit well below our band, and they are no longer the only thing in view.

### Verification, as at `P-71`

Citations are now confirmed against **Crossref**, and abstracts read **verbatim from primary indexes** — ERIC for the two journal studies, APA PsycNet for the meta-analysis — rather than from search summaries. All three full texts are closed: Semantic Scholar reports no open-access PDF, and the publishers return 403/402.

So the standard is now *publisher abstract*, which is well above the secondary summaries `P-07` used and still below `P-06`'s "read the source".

**Two corrections came out of reading the real abstracts.**

**1 · The age moderator is weaker than recorded.** `P-07` wrote that the effect is "larger for adult-only samples" and that "the age moderator runs against us". What the abstract actually says is that the weighted mean effect size was larger for studies *"conducted in laboratories with adult-only samples (ranging in size from 101 to 200 participants) that used deep learning strategies, prediction and postdiction confidence judgments, the difference between prediction–postdiction judgments to calculate monitoring accuracy, and multiple-choice response options"*. That is **one compound description of which studies showed larger effects**, not age isolated as a moderator. Age may still run against us; the abstract does not establish it.

**2 · Duration did not moderate, and that had been missed entirely.** The same sentence continues: weighted mean effect sizes for the type of monitoring measure, research design, **learning strategy instruction duration**, and geographic location *"did not vary significantly among studies"*. `R-005` claims improvement **in six weeks**, and duration not moderating is the closest thing to support that claim has. It was invisible in the summary `P-07` worked from.

**And one limit that applies to all of it:** the meta-analysis is about **learning strategy instruction** against a control. We do not teach learning strategies. It bounds the field, not our intervention.
