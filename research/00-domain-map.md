...
4. [Bibliography](99-bibliography.md) — sources, with verification status.

Then the spec: [Problem Statement](../spec/01-problem-statement.md) → [Phase 0 Requirements](../spec/02-requirements.md) → [Pilot Design](../spec/03-pilot-design.md).

## The five domains this project sits across

| Domain | What we need from it | Status |
|---|---|---|
| **Misconception research** | The item bank and the error taxonomy. Force and motion specifically. | Identified, unverified — `P-06` |
| **Memory and practice design** | Spacing, interleaving, retrieval, generation, desirable difficulties. | Identified, unverified — `P-06` |
| **Metacognition and calibration** | Confidence capture, Brier scoring, hypercorrection. Weakest for our age band. | Gap — `P-07` |
| **Instructional design counter-position** | Cognitive load, worked examples, expertise reversal, the Kirschner critique. | Identified; must be engaged, not dismissed |
...
**Our own extrapolations, unproven.** That per-learner error signatures are *stable across topics* (`R-002`). That calibration is trainable in 11–15 year olds and transfers (`R-005`). That baiting a signature beats routing around it (`R-003`). These three are the project. They are also the three things not to state as fact anywhere outside this repository.

**Known gap we have not addressed.** Expertise reversal cuts directly against error-signature baiting. Phase 0 does not test it. See [03 · Personalisation Axes](03-personalisation-axes.md).
